...
# Copied from https://github.com/jlm429/bettermdptools - to edit the plot_policy
...
class Plots:
    @staticmethod
    def values_heat_map(data, title, size, ax=None):
        data = np.around(np.array(data).reshape(size), 2)
        df = pd.DataFrame(data=data)
        sns.heatmap(df, annot=True, ax=ax).set_title(title)
    ...
    #modified from https://gymnasium.farama.org/tutorials/training_agents/FrozenLake_tuto/
    @staticmethod
    def plot_policy(val_max, directions, title, ax=None):
